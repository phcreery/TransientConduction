...
if True:
    ...
    for t in range(0, ti):
        ...
        T[0, t+1] = T[0, t] + diffusionNumber * \
            (T[1, t] - T[0, t] + Qin*(dx)/(k*Ai))
        T[Li, t+1] = T[Li, t] + diffusionNumber * \
            (T[Li-1, t] - T[Li, t] - Qout*((dx)/(k*A)))

        for x in range(int(dx*xscale), int((L-dx)*xscale)+1, int(dx*xscale)):
            T[x, t+1] = T[x, t] + diffusionNumber * \
                (T[x-1, t] - 2*T[x, t] + T[x+1, t])
...
if True:
    ...
    for t in range(0, ti):
        ...
        T[:, t+1] = np.concatenate((np.array([T[0, t+1]]),
                                   Tsub, np.array([T[Li, t+1]])))
